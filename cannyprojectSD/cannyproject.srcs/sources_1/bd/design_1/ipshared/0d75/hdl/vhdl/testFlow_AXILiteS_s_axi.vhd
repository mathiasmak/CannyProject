...
entity testFlow_AXILiteS_s_axi is
generic (
...
port (
    ...
    b_ce0                 :in   STD_LOGIC;
    b_we0                 :in   STD_LOGIC;
    b_d0                  :in   STD_LOGIC_VECTOR(7 downto 0)
);
end entity testFlow_AXILiteS_s_axi;
...
architecture behave of testFlow_AXILiteS_s_axi is
    ...
    constant ADDR_B_HIGH  : INTEGER := 16#40f#;
    ...
    component testFlow_AXILiteS_s_axi_ram is
        generic (
            BYTES   : INTEGER :=4;
            DEPTH   : INTEGER :=256;
            AWIDTH  : INTEGER :=8);
    ...
    end component testFlow_AXILiteS_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_a
int_a : testFlow_AXILiteS_s_axi_ram
generic map (
     BYTES    => 4,
     DEPTH    => 113,
     AWIDTH   => log2(113))
port map (
     ...
     q1       => int_a_q1);
-- int_b
int_b : testFlow_AXILiteS_s_axi_ram
generic map (
     BYTES    => 4,
     DEPTH    => 3,
     AWIDTH   => log2(3))
...
-- ----------------------- Memory logic ------------------
    -- a
    int_a_address0       <= SHIFT_RIGHT(UNSIGNED(a_address0), 2)(6 downto 0);
    int_a_ce0            <= a_ce0;
    int_a_we0            <= a_we0;
    ...
    -- b
    int_b_address0       <= SHIFT_RIGHT(UNSIGNED(b_address0), 2)(1 downto 0);
    int_b_ce0            <= b_ce0;
    int_b_we0            <= b_we0;
    int_b_be0            <= SHIFT_LEFT(TO_UNSIGNED(1, 4), TO_INTEGER(UNSIGNED(b_address0(1 downto 0))));
    int_b_d0             <= UNSIGNED(b_d0) & UNSIGNED(b_d0) & UNSIGNED(b_d0) & UNSIGNED(b_d0);
...
entity testFlow_AXILiteS_s_axi_ram is
    generic (
        BYTES   : INTEGER :=4;
        DEPTH   : INTEGER :=256;
        AWIDTH  : INTEGER :=8);
...
end entity testFlow_AXILiteS_s_axi_ram;

architecture behave of testFlow_AXILiteS_s_axi_ram is
